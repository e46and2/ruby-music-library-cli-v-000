...
def list_songs
  songs = []
  songs << Song.all.sort{ |a, b| a.name <=> b.name }.each.with_index(1) do |s, i| puts "#{i}. #{s.artist.name} - #{s.name} - #{s.genre.name}"
    #binding.pry
      end
      songs 
#binding.pry
...
def list_genres
  ...
  def play_song
    ...
    if (input > 0) && (input <= list_songs.songs.size)
      song = list_songs.songs[input+1]
